...
def ens_plot_all(train_set, test_gen, model_ens):
    ...
    pdb.set_trace()
    plt.clf()
    plt.scatter(test_X.numpy()[test_order], test_y.numpy()[test_order], s=2)
    ...
    pdb.set_trace()
    plt.clf()
    plt.scatter(test_X.numpy()[test_order], test_y.numpy()[test_order], s=3)
    plt.plot(test_X.numpy()[test_order], pred_mean[test_order])
    plt.fill_between(test_X.numpy()[test_order].flatten(),
                     pred_mean[test_order].flatten() - 2 * test_pred_stds[test_order].flatten(),
                     pred_mean[test_order].flatten() + 2 * test_pred_stds[test_order].flatten(), alpha=0.5)
    plt.show()